...
public class Function {
    ...
    public HttpResponseMessage getCVPageCount(@HttpTrigger(name = "req", methods = { HttpMethod.GET }, authLevel = AuthorizationLevel.ANONYMOUS)
                                                HttpRequestMessage<Optional<String>> request,
            @CosmosDBInput(name = "pagecounterdb", databaseName = "pagecounterdb", collectionName = "pagecounterdbcontainer", id = "{Query.id}", partitionKey = "{Query.id}", connectionStringSetting = "CosmosDbConnectionString") 
                                                Optional<String> item,
            @CosmosDBOutput(name = "pagecounterdbupdate", databaseName = "pagecounterdb", collectionName = "pagecounterdbcontainer", connectionStringSetting = "CosmosDbConnectionString") 
        ...
        if (count != null) {
            ...
            HttpResponseMessage resp = request.createResponseBuilder(HttpStatus.OK).body("Count not retrieved.").build();
            return resp;
        }
    }
